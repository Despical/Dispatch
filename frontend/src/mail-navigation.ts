...
export function readMailLocation(url: URL): MailLocation {
  ...
  return {view,accountId:segment.startsWith('accounts/')?id(segment.slice(9)):view==='trash'||view==='drafts'?id(url.searchParams.get('account')):null,folderId:view==='inbox'?id(url.searchParams.get('folder')):null,
...
export function mailLocationUrl(route: MailLocation): string {
  ...
  if((route.view==='trash'||route.view==='drafts')&&route.accountId)params.set('account',String(route.accountId));
